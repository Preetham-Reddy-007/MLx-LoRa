...
% 	fig(2) = figure(2);
	...
	for SF = SF_min:SF_max
		...
		fc = 0;
		phi0 = 2*pi*rand;
		c = lora_chirp(fc, mu_updn_sel, BW, SF, 0, 0, OSF);
...
% 			mu_updn_sel = +1;
			fc_int = 0;
			phi0_int = 2*pi*rand;
			c_int = lora_chirp(fc_int, mu_updn_sel, BW_int, SF_int, 0, phi0_int, OSF_int);
			...
			while SINR_idx<=numel(SINR_dB) && seconds+toc<=seconds_budget
				...
				while rx_errors_tot < rx_errors_threshold ...
                    ...
                    mu_updn_sel_inv = -1*mu_updn_sel;
					down_chirp = downchirp(fc, mu_updn_sel_inv, BW, SF, 0, 0, OSF);
					for sym = 1:N_syms
						temp = down_chirp.*r((sym-1)*sym_len+(1:sym_len));
                        if mu_updn_sel == 1
						    z = fft(temp(1:OSF:end));
                        elseif mu_updn_sel == -1
                            z = fft(temp(1:OSF:end));
                        end
						[max1,pos] = max(abs(z));
...
% 					SINR_dB(SINR_idx),Pe,SER,BER,trials)
		...
		for SF_int = SF_int_min:SF_int_max
			l_entries{SF_int-SF_int_min+1} = ['MI',num2str(SF_int)];
		end
		legend(l_entries)
		ylim([BER_target,1])
		
    end
    save('test_lora_Phy_1.mat')
% end
...
function [ s, t ] = lora_chirp(fc, mu, BW, SF, k, phi0, OSF)
    ...
    if mu == 1
        % UP Down signal
        freq_orig = mod(S/(Ts*mult) + t.*(BW/(Ts*mult)),BW) - BW/2;
        freq_inv = mod((M-S)/(Ts*mult) - t.*(BW/(Ts*mult)),BW) - BW/2;
        freq = [freq_orig(1:length(freq_inv)/2); freq_inv(length(freq_inv)/2+1:length(freq_orig))];
%         freq_orig(1:length(freq_inv)/2)
        sel = randi(2)
        disp(sel)
        if sel == 1
            y_final = cumtrapz(t, freq);
        else
            y_final = cumtrapz(t, freq_orig);
        end
...
% y = cumtrapz(t, freq_orig);
% y_inv = cumtrapz(t, freq_inv);
% y_final = cumtrapz(t, freq);
% 
% 
% % signal generated
% sig = 2*exp(1j*2*pi*y);
% sig_inv = 2*exp(-1j*2*pi*y_inv);
% s = 2*exp(1j*2*pi*y_final);
% 
% end

function [ s, t ] = downchirp(fc, mu, BW, SF, k, phi0, OSF)
    M = 2^SF;
    Ts = (2^SF) / BW;
    Fs = OSF*BW;
    Ns = round(Fs/BW*M);
